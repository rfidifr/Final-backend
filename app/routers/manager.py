...
@router.post("/create_card",status_code=status.HTTP_201_CREATED,response_model=schemas.CardResponse)
def create_card(
    ...
    new_card = models.Card(**card_data.model_dump(), arcade_id=current_user.arcade_id)
    db.add(new_card)
    try:
         db.commit()
         db.refresh(new_card)
        ...
        raise HTTPException(
            ...
            detail="Card ID already exists. Please use a unique card_id."
        )

    return {
    "message": "card created successfully",
    "card_id": new_card.card_id,
    "owner_name": new_card.owner_name,
    "contact_no": new_card.contact_no,
    "balance": new_card.balance
    }


@router.get("/view_card/{card_id}")
def view_card(
    card_id: str,
    db: Session = Depends(database.get_db),
    current_user: models.User = Depends(get_current_user)
